...
{
    public class OrientationSpecial : Algorithm
    {
        ...
        public bool specialMirror = false;
        ...
        void SetParsedDataNull()
        {

            //if (orientationData.Length < 1) return;
            ...
            if (specialMirror)
            {
              //this works  transform.rotation = new Quaternion(orientation.x, transform.rotation.y, orientation.z, orientation.w);
                var quatFix = new Quaternion(orientation.x, transform.rotation.y, orientation.z, orientation.w);
                transform.rotation = quatFix; 
                transform.Rotate(0f,  orientation.z/180.0f, 0f, Space.Self);
            }
        }
